...
就是我們最後插入的那一筆數據項，
如果我們想要訪問更先前插入的數據項，
我們必須先移除這一筆數據項。因此，堆疊在在程式中更多時候被擁來當作工具，而不是真正用來儲存資料。
...
堆疊最主要是透過陣列來實現，陣列stackArray是我們實現堆疊的陣列，maxSize代表著類別中陣列的最大長度，也是我們堆疊的最大長度。
top代表著是堆疊中最上層的位置，也是陣列最後端的位置，他是唯一可以被查找的位置，當然他的數字在陣列有新增或刪除的動作時會改變。
...
在我們新增一比值到堆疊時，類別屬性top就會增加，除了用來讓當作索引讓新的值放入陣列，他也代表著堆疊中可以查找的位置。
...
當我們在這堆疊中刪除一個值時，我們並沒有真正的執行刪除或是取代的動作。我們唯一做的就是top--，和返回那一筆值(這對那一比值沒有影響)。
但是這部會影響我們接下來的動作，因為在下次做新增的動作時，原本的直就會被覆蓋過去。
...
這是我們用來訪問目前最上層數據的韓式，他永遠是反維陣列中top位置的值，而top的值永遠隨著陣列長度變動，
所以可訪問的直永遠是陣列最後面的值。
