...
選擇排序法改進了冒泡排序法，將交換次數從O(N^2)改進到O(N)，但是比較次數依然維持在O(N^2)。
對於大記錄量的的排序來說，選擇排序法做了很重要的改進。因為一般來說，執行交換時會需要移動這些紀錄內存的位置，
所以交換比比較更重要，而選擇排序法就改善了交換的次數。
...
在每一圈的out迴圈中我們只執行一次交換，所以交換的次數永遠是N-1。

外迴圈的指標out就代表當內回圈結束時變數min需要做交換的目標位置，
而內回圈的指標in就代表著需要比較的數組的起始位置。
從左邊開始比較，指標小於out的數字將會比out右邊的數字都小，而且有序。
...
而第二圈中(此時數字0的位置已經不再進行比較)，數字1從第三位的位置被移動到了倒數第二的位置。
在接下來的每一圈外迴圈，相對應的數字都會透過交換被移動到個別相對應的位置。


在效率上，選擇排序法和冒泡排序法所需要的比較次數是一樣的N*(N-1)/2，
但是在交換上，以十個數據項為例，冒泡排序法最多會需要45次交換，而選擇排序法只需要9次。
在交換比比較需要消耗更多時間狀況下，選擇排序法會比冒泡排序法快得多。
*/
