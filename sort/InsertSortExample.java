...
/*
插入排序法是三種簡單排序法中最好的一種，他比冒泡排序法快一倍，比選擇排序法快一點。在某些狀況下他也會被使用在快速排序法中處理比較小的組數。
...
public void insertionSort()
{
	...
	for ( int out = 1; out<numValues; out++) //從第一位遞增往後跑
	{
		long temp = arrInsert[out];//存入暫存變數	
		int in = out;
		while(in > 0 && arrInsert[in-1] >= temp)//所有的數都跟temp比較,比temp大就往後移,直到沒有後將temp放入那一位	
		{
			arrInsert[in] = arrInsert[in-1];
			--in;
		}
...
那麼他前面的數就會在比較後一個個的往後移一位，然後out指標位置的值就會被插入第一位的位置。
但是如果out指標位置的值在中途就遇到了比它小的值，
...
在插入排序法中，第一趟他需要最多一次比較，第二趟需要最多兩次比較。依此類推到最後一趟，她需要N-1次的比較。
所以會是1+2+3_4.....+N-1 = N*(N-1)/2。複製的次數基本等於比較的次數。但是，一次複製與一次交換所消耗的時間不同，
所以他比冒泡排序快一倍，比選擇排序快一點。
對於已經有序或基本有序的數據來說，使用插入排序是最好的。當遇到有序的數據時，插入排序的while迴圈條件並不會執行，
他只會負責跑完N-1圈，所以運行的時間是O(N)，所以在對基本有序的數據時，插入排序是更好的選擇。
而在冒泡排序或著選擇排序，即使在在面對有序的數據時，冒泡排序和選擇排序的內圈依舊會執行比較。

但是對於逆序排列的數據時，每次的比較和移動對會執行，所以並不會比冒泡排序法快多少。
*/
